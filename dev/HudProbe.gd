...
##     hud.fits      the room tag, tracker, item card, carry bar, prompt, notice and overview end inside the screen
##     hud.clear     none of those meet each other; the overview meets neither the notice, the carry bar nor the room tag
...
##                   its cost, outlines the last one taken and names it, and never grows past its width;
##                   at the start of a run, one slot and one item, the name it puts back is not cut off
##                   and the slot count is singular
##
...
func _check(screen: Vector2i, content: Catalogue, plan: FloorPlan, pass_tag: String) -> void:
	...
	for part: String in ["%RoomTag", "%Tracker", "%Card", "%CarryBar", "%Prompt", "%Notice"]:
		var control := hud.get_node(part) as Control
		if control.is_visible_in_tree():
			parts[part] = control.get_global_rect()
	_ok("hud.fits", parts.size() == 6, "%s: shown %s" % [tag, parts.keys()])
...
func _check_bar(hud: Hud, tag: String, logical: Vector2i) -> void:
	...
	_ok("hud.bar", bar.used_text() == NumberFormatter.slots_of(used, Inventory.capacity)
			...
			and bar.back_text().contains(tr(carried.back().name_key))
			...
			bar.back_text(), bar.cells().get_rect()])
