...
Every set is every copy of one item type, except a set listed in KINDS, whose members are a few kinds of one
family (the table tennis gear). It writes the generated half of docs/CONTENT.md:
...
ITEMS = [
    ...
    # Rec room, added after the list was scattered, so the draws for every set above stay as they were
    ("table_tennis", "Table tennis equipment", 6, 0.25, 2, "rec_room", "table tennis table", "two bats, three balls, the net", 3),
]

# A set whose members are not all one thing: (kind, name, count, kg each, bulk tier), in the order their
# copies are numbered. The row in ITEMS carries the heaviest and bulkiest kind, which is what a trip is planned by.
# The author, 2026-09-17: the bats, the ball and the net had been built into the table.
KINDS = {
    "table_tennis": [
        ("bat", "Table tennis bat", 2, 0.09, 1),
        ("ball", "Table tennis ball", 3, 0.003, 1),
        ("net", "Table tennis net", 1, 0.25, 2),
    ],
}

def member_costs(it):
    """The slot cost of every member of a set, in copy order."""
    if it[0] not in KINDS:
        return [cost(it[3], it[4])] * it[2]
    return [cost(k[3], k[4]) for k in KINDS[it[0]] for _ in range(k[2])]
...
def run():
    for key, kinds in KINDS.items():
        row = next(i for i in ITEMS if i[0] == key)
        assert sum(k[2] for k in kinds) == row[2], f"{key}: its kinds do not add up to its count"
...
if __name__ == "__main__" and "--md" not in sys.argv and "--json" not in sys.argv:
    ...
    costs = [c for i in ITEMS for c in member_costs(i)]
    ...
    for floor, zones in FLOORS.items():
        ...
        for z in zones:
            ...
            for i in its:
                ...
                each = sorted(set(member_costs(i)))
                w(f"| {i[1]} | {i[2]} | {' or '.join(str(c) for c in each)} | {i[6]} | {i[7] or '-'} | {cells} |")
    w("")
    w("## The absurd spots")
    w("")
...
def plan_json():
    ...
    for it in ITEMS:
        ...
        out.append({
            ...
            "absurd": absurd[1],
            "kinds": [{"kind": k[0], "name": k[1], "count": k[2], "kg": k[3], "slot_cost": cost(k[3], k[4])}
                    for k in KINDS.get(it[0], [])],
        })
