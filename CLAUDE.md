...
## Verification — never say "it works" without one of these
    ...
    godot --headless --path . dev/InteractProbe.tscn         # pick up, refusals, open, put away, sorted stays sorted, stack, select, drop, throw, lost items, hands
    godot --headless --path . dev/HomeProbe.tscn             # every item put into its own home by pointing and pressing, and in reach to take out
    godot --headless --path . dev/DropProbe.tscn             # every item type dropped onto a floor lies on it, not in it
    godot --headless --path . dev/FurnitureProbe.tscn        # furniture placement, clear fronts, doors swinging clear, groups, winding
    godot --headless --path . dev/HudProbe.tscn              # HUD layout 720p-1440p and +40% text; card, bar, compass, ledger, its clicks
    ...
    godot --headless --path . dev/WayProbe.tscn              # markers lead home; the home is outlined through the walls; a set looked for
