...
| Scale | ~2 storeys + basement + attic + garage + garden/deck/pool. **25 zones, ~250 item instances, 56 sets: one per type, and the table tennis gear as one set of bats, balls and the net** (the author, 2026-09-17). Derived in `docs/HOUSE.md`, not picked. |
...
| Set completion | On **placing** every member at home, not on finding them. An item standing in its own home is sorted and cannot be picked up again (the author, 2026-09-17): the crosshair says so, and nothing the player presses undoes their own work. It can still be put down in a group that is not its home, and taken back from there. |
| Findability | **Set tracker plus per-room clutter counts.** On screen at all times: sets complete, the sets under way, and the misplaced count of the room you stand in. A key opens the ledger: every set with how many members remain and every room that still holds misplaced items (the author, 2026-09-14: 55 sets did not fit beside the game). Never which item, never where it lies **unless the player asks**: picking one set in the ledger outlines its misplaced members through the walls, wherever they lie (the author, 2026-09-17, "for full convenience and comfort"). One set at a time, and off by default, so the hunt is still the game. Once found, an item says where it belongs: its room and the piece (the HUD plan, 2026-09-16). Carried, it is guided there (decision D1, the author, 2026-09-16): a compass marker for the item in hand aimed along the route, with the floor it is on, and the piece outlined through the walls and pinned from anywhere in the house (the author, 2026-09-17). A "names only" setting turns the guidance off, but not the set the player asked for. |
| UI | **Shown, not written.** A thing is its picture, progress is pips and "3/12", and a sign never stands without a word beside it. Each figure is written once, where the player acts on it, and a shade, a bar or a row of pips carries the rest. Detail is for the one thing in question, and a refusal names the fact that refused it. Nothing is said by colour alone, text is at least 18 px at 1080p, and nothing is held down that could be toggled (the author, 2026-09-17; `docs/ARCHITECTURE.md`, "Reading the HUD"). |
