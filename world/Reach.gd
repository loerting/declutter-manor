class_name Reach
## Whether a player in the house, standing or crouched, could pick an item up where it is. `ContentImport` keeps
...
## True when a player could pick up an item whose bounds are `box` at `xform`: some eye at standing or
## crouching height over `floor_y`, within reach, with nothing solid at its eye or its feet, sees the item
## before anything that stops the interaction ray. Other items do not count: the one in front is picked up
## first. A crouched eye counts because the player can crouch (`PlayerController`): a pair of sneakers in the
## oven is found by looking into it.
static func reachable(space: PhysicsDirectSpaceState3D, plan: FloorPlan, box: AABB, xform: Transform3D,
	...
	for height: float in [Balance.EYE_HEIGHT, Balance.CROUCH_EYE_HEIGHT]:
		for step in range(TURNS):
			...
			for out: float in OUT:
				...
				var eye := Vector3(feet.x, floor_y + height, feet.z)
...
static func _solid_at(space: PhysicsDirectSpaceState3D, at: Vector3) -> bool:
	...
	q.position = at
	q.collision_mask = Layers.body_mask()
