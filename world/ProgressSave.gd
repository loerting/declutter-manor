...
## The save, as the world stands. A carried item is saved where it was picked up from, which is
## where putting it back would put it: the hands are not a place, and a quit mid-trip loses
...
static func _entry(item: ItemNode, plan: FloorPlan) -> Dictionary:
	var slots := item.origin_slots if item.is_carried() else item.get_parent() as PlaceSlots
	var index := item.origin_index if item.is_carried() else (slots.slot_of(item) if slots != null else -1)
	var parent := item.origin_parent if item.is_carried() else item.get_parent() as Node3D
	var local := item.origin_xform if item.is_carried() else item.transform
...
static func apply(root: Node, items_root: Node3D, plan: FloorPlan, content: Catalogue,
	...
	for item: ItemNode in items_in(root):
		...
		if container_id != &"":
			...
			if container != null:
				_move(item, container, xform as Transform3D)
			continue
